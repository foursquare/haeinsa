 ...
 * include information of overall transaction, and have reference to {@link
 * HaeinsaTransactionManager} which created this instance.
 *
 * <p>HaeinsaTransaction can be generated via calling {@link HaeinsaTransactionManager#begin()} or
 * {@link HaeinsaTransactionManager#getTransaction(byte[], byte[])}. Former is used when start new
 * transaction, later is used when try to roll back or retry failed transaction.
 *
 * <p>One {@link HaeinsaTransaction} can't be used after calling {@link #commit()} or {@link
 * #rollback()} is called.
 */
public class HaeinsaTransaction {
  ...
  private static enum CommitMethod {
    ...
    SINGLE_ROW_PUT_ONLY,
    /**
     * When there is multiple rowTx and at least one of that include mutation, or there is only one
     * rowTx and its mutation contains HaeinsaDelete.
     */
    MULTI_ROW_MUTATIONS,
  ...
  /**
   * Indicate whether this transaction has any changes. This method return true if the transaction
   * contains one or more {@link HaeinsaPut} or {@link HaeinsaDelete} operation.
   *
  ...
  /**
   * Bring {@link HaeinsaTableTransaction} which have name of tableName. If there is no {@link
   * HaeinsaTableTransaction} have this name, then create one instance for it and save inside {@link
   * #txStates} and return.
   *
  ...
  /**
   * Commit transaction to HBase. Once succeed, data will applied to HBase permanently. If the
   * transaction conflict with other concurrent transaction, entire data will be failed, and throw
   * {@link ConflictException}.
   *
   * <p>Once this method is invoked, this instance is not usable anymore. If invoked twice, {@link
   * IllegalStateException} will thrown.
   *
   * @throws IOException Error during executing HBase operation or {@link ConflictException} during
   *     commit operation.
   */
  public void commit() throws IOException {
    ...
    // Determine maxCurrentCommitTimestamp and maxIterationCount, from all participating rows of
    // transaction.
    ...
    for (Entry<byte[], HaeinsaTableTransaction> tableStateEntry :
        txStates.getTableStates().entrySet()) {
      for (Entry<byte[], HaeinsaRowTransaction> rowStateEntry :
          tableStateEntry.getValue().getRowStates().entrySet()) {
        ...
        maxCurrentCommitTimestamp =
            Math.max(maxCurrentCommitTimestamp, rowState.getCurrent().getCommitTimestamp());
      }
    }

    // The prewriteTimestamp of the transaction should bigger than any other commitTimestamps of
    // rows in the
    // transaction.
    // Written data can be disappear randomly if write with same timestamp during major compaction.
    // (It is because of algorithm which is used for determining newest data in multiple HFiles
    // during major
    // compaction)
    ...
    // If some row contains multiple mutations, several timestamps are used in applying mutations to
    // row
    // sequentially.
    // So, we have to determine commitTimestamp properly.
    setCommitTimestamp(
        Math.max(getPrewriteTimestamp() + 2, maxCurrentCommitTimestamp + maxIterationCount + 2));

    extendExpiry();

    TRowKey primaryRowKey = null;
    NavigableMap<TRowKey, HaeinsaRowTransaction> mutationRowStates =
        txStates.getMutationRowStates();
    NavigableMap<TRowKey, HaeinsaRowTransaction> readOnlyRowStates =
        txStates.getReadOnlyRowStates();
  ...
  /**
   * Use {@link HaeinsaTable#checkSingleRowLock(HaeinsaRowTransaction, byte[])} to check RowLock on
   * HBase of read-only rows of tx. If all lock-checking by get was success, read-only tx was
   * success. Throws ConflictException otherwise.
   *
  ...
  private void commitReadOnly() throws IOException {
    ...
    // check secondaries
    for (Entry<TRowKey, HaeinsaRowTransaction> rowKeyStateEntry :
        txStates.getReadOnlyRowStates().entrySet()) {
  ...
  private void commitMultiRowsMutation() throws IOException {
    ...
    for (Entry<TRowKey, HaeinsaRowTransaction> rowKeyStateEntry :
        txStates.getMutationRowStates().entrySet()) {
      TRowKey key = rowKeyStateEntry.getKey();
      HaeinsaRowTransaction rowTx = rowKeyStateEntry.getValue();
      if (Bytes.equals(key.getTableName(), primary.getTableName())
          && Bytes.equals(key.getRow(), primary.getRow())) {
  ...
  /**
   * Change states of {@link TRowLock} of all mutation rows to {@link TRowLockState#STABLE}. This
   * can be called by following two cases.
   *
   * <p>1. In case of {@link #commitMultiRowsMutation()}, after changing primary row to {@link
   * TRowLockState#COMMITTED} and applying all mutations in primary row and secondary rows.
   *
   * <p>2. When try to {@link #recover(boolean)} failed transaction in the middle of execution. This
   * method should be called only when primary row is in the state of {@link
   * TRowLockState#COMMITTED}.
   *
   * @throws IOException ConflictException, HBase IOException.
   */
  private void makeStable() throws IOException {
    ...
    HaeinsaTablePool tablePool = getManager().getTablePool();
    HaeinsaRowTransaction primaryRowTx =
        createOrGetTableState(primary.getTableName()).createOrGetRowState(primary.getRow());
    ...
    try {
      ...
      // if making row stable is failed, but primary is committed. Then treat this transaction as
      // succeeded.
  ...
  /**
   * Reload information of failed transaction and complete it by calling {@link #makeStable()} if
   * already completed one, ( when primaryRow have {@link TRowLockState#COMMITTED} state } or abort
   * by calling {@link #abort()} otherwise.
   *
   ...
   * Transaction can be canceled by client which started it when failed to acquire lock of mutation
   * row, or by other client which try to access any row of failed transaction which have past
   * expiry. Aborting failed transaction is basically processed by lazy-recovering.
   *
   * <p>When try to roll back failed transaction started by other client, this method assume that
   * state of failed transaction is properly loaded from locks of primary and secondary rows to
   * {@link #txStates} of this instance.
   *
   * <p>Aborting is executed by following order.
   *
   * <ol>
   *   <li>Abort primary row by calling {@link
   *       HaeinsaTableIfaceInternal#abortPrimary(HaeinsaRowTransaction, byte[])}.
   *   <li>Visit all secondary rows and change from prewritten to stable state. Prewritten data on
   *       rows are removed at this state.
   *   <li>Change primary row to stable state.
   * </ol>
   *
   * @throws IOException ConflictException, HBase IOException.
   */
   ...
   *
   * <p>This class is not Thread-safe. This class will separate each {@link HaeinsaRowTransaction}
   * to ReadOnlyRowStates and MutationRowStates.
   *
   * <p>If rowState have more than 1 mutations or state of row is not {@link TRowLockState#STABLE},
   * then that row is MutationRow. ReadOnlyRow otherwise (There is no mutations, and state is
   * STABLE).
   */
  private static class HaeinsaTransactionState {
    private final NavigableMap<byte[], HaeinsaTableTransaction> tableStates =
    ...
    /**
     * Check whether there is row which contains mutation. This method returns false only if there
     * is no changes on data in the transaction.
     *
     * @return true if one or more row state has mutation, false if any row does not contains
     *     mutation.
     */
    public boolean hasChanges() {
    ...
    /**
     * Determine commitMethod among {@link CommitMethod#READ_ONLY}, {@link
     * CommitMethod#SINGLE_ROW_PUT_ONLY} and {@link CommitMethod#MULTI_ROW_MUTATIONS}
     *
  ...
  /**
   * Basic comparator which use lexicographical ordering of (byte[] table, byte[] row). Thread-safe
   * & stateless
   */
   ...
   *
   * <p>Get guava murmur3_32bit hash value of (byte[] table, byte[] row), and compare those two to
   * order {@link TRowKey}
   */
