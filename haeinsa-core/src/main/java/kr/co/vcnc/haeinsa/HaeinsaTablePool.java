...
public class HaeinsaTablePool implements Closeable {
   ...
   */
  public HaeinsaTablePool(
      final Configuration config, final int maxSize, final HaeinsaTableIfaceFactory tableFactory) {
  ...
  public HaeinsaTablePool(
      ...
      final int maxSize,
      final HaeinsaTableIfaceFactory tableFactory,
      PoolType poolType) {
    ...
    this.tableFactory =
        tableFactory == null ? new DefaultHaeinsaTableIfaceFactory(connection) : tableFactory;
    if (poolType == null) {
  ...
  /**
   * Closes all the HaeinsaTable instances , belonging to the given table, in the table pool.
   *
   * <p>Note: this is a 'shutdown' of the given table pool and different from {@link
   * #release(HaeinsaTableIface)}, that is used to return the table instance to the pool for future
   * re-use.
   */
   ...
   *
   * <p>Note: this is a 'shutdown' of all the table pools.
   */
  @Override
  ...
  class PooledHaeinsaTable implements HaeinsaTableIfaceInternal {
    ...
    @Override
    public HaeinsaResultScanner getScanner(HaeinsaTransaction tx, HaeinsaIntraScan intraScan)
        throws IOException {
    ...
    @Override
    public void prewrite(HaeinsaRowTransaction rowTxState, byte[] row, boolean isPrimary)
        throws IOException {
